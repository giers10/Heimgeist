...
def run_isolated(func, path, args, *, timeout=900, cancellation_event: Optional[mp_context.Event] = None):
    """
    Run a CPU/IO-heavy function in a child process.
    If the child segfaults, times out, or crashes, we return a synthetic error.
    """
    ctx = mp_context.get_context("fork" if sys.platform == "darwin" else "spawn")
    ...
    try:
        ...
        while p.is_alive():
            if cancellation_event and cancellation_event.is_set():
                ...
                p.join()
                return [], "cancelled"
            if parent_conn.poll(0.1): # Check for data with a small timeout
                status, payload = parent_conn.recv()
                break
            time.sleep(0.1) # Small sleep to prevent busy-waiting
        else: # Process died without sending data
            if not cancellation_event or not cancellation_event.is_set():
                # Only report crash if not cancelled
                status, payload = ("err", "crash")
...
try:
    mp_context.set_start_method("fork")
except RuntimeError:
    pass
...
def run_build(root: Path, out: Path, *, on_progress=None, **opts) -> dict:
    global LLM_SEM
    # Use provided options or default values
    args = argparse.Namespace(
        root=root,
        out=out,
        workers=opts.get("workers", os.cpu_count() or 4),
        verbose=opts.get("verbose", False),
        emit=opts.get("emit", "auto"),
        emit_av=opts.get("emit_av", "joined"),
        ocr_page_jobs=opts.get("ocr_page_jobs", 1),
        ocr_lang=opts.get("ocr_lang", "eng"),
        max_cols=opts.get("max_cols", 4),
        epub_strategy=opts.get("epub_strategy", "pdf-fallback"),
        pdf_page_workers=opts.get("pdf_page_workers", 0),
        html_section_workers=opts.get("html_section_workers", 0),
        include=opts.get("include", r".*\.(?:pdf|html?|txt|md|rst|epub|png|jpe?g|gif|bmp|tiff?|webp|heic|mp3|wav|m4a|flac|ogg|opus|aac|mp4|mkv|mov|webm|avi|ts|py|ipynb|js|ts|tsx|jsx|java|c|cpp|rs|go|rb|php|cs|swift|kt|m|sh|bat|ps1|sql)$"),
        exclude=opts.get("exclude", r"(^|[\\/])\.|__MACOSX([\\/]|$)|\.DS_Store$|\.ocr\.txt$"),
        whisper_model=opts.get("whisper_model", "base"),
        num_slices=opts.get("num_slices", 8),
        overlap_sec=opts.get("overlap_sec", 1.0),
        max_overlap_words=opts.get("max_overlap_words", 7),
        mp_workers=opts.get("mp_workers", 0),
        asr_task=opts.get("asr_task", "transcribe"),
        max_av_duration=opts.get("max_av_duration", 5 * 3600),
        whisper_device=opts.get("whisper_device", "auto"),
        ollama_host=opts.get("ollama_host", "http://localhost:11434"),
        vlm_model=opts.get("vlm_model", "qwen2.5vl:7b"),
        code_llm=opts.get("code_llm", "qwen3:4b"),
        llm_parallel=opts.get("llm_parallel", 1),
        image_max_edge=opts.get("image_max_edge", 1600),
        image_text_gate=opts.get("image_text_gate", "tesseract-conf"),
        ocr_psms=opts.get("ocr_psms", "6,11"),
        ocr_min_conf=opts.get("ocr_min_conf", 55),
        ocr_min_words=opts.get("ocr_min_words", 10),
        ocr_min_alnum=opts.get("ocr_min_alnum", 0.55),
        code_max_bytes=opts.get("code_max_bytes", 200_000),
        lang_hint=opts.get("lang_hint", None),
        lang_detect=opts.get("lang_detect", True),
    ...
    if on_progress:
        on_progress("start", 0.0, f"Writing JSONL to: {out_path}")

    include_rgx = re.compile(args.include, flags=re.I)
    exclude_rgx = re.compile(args.exclude, flags=re.I)

    if on_progress:
        on_progress("scan", 0.05, "Scanning files...")
    files = list(iter_files(root, include_rgx, exclude_rgx))
    if not files:
        if on_progress:
            on_progress("done", 1.0, "No matching files found.")
        stop_writer()
        return {"status": "warning", "message": "No matching files found."}
    ...
    if on_progress:
        on_progress("extract", 0.1, f"Extracting content from {total} files...")
    ...
    with cf.ThreadPoolExecutor(max_workers=max(1, args.workers)) as ex:
        ...
        for fut in cf.as_completed(futures):
            ...
            if on_progress:
                pct = 0.1 + 0.8 * (processed_count / total)
                on_progress("extract", pct, f"Processed {processed_count}/{total}: {path.name}")

    if on_progress:
        on_progress("write", 0.9, "Finalizing write...")
    stop_writer()
    if on_progress:
        on_progress("done", 1.0, "Corpus build complete.")

    return {"status": "ok", "total_files": total, "errors": errors}

def main():
    args = parse_args()
    root_arg = args.root or args.mirror
    if not root_arg:
        print("[ERROR] Please provide --root <dir>.", file=sys.stderr)
        sys.exit(2)
    
    run_build(Path(root_arg), Path(args.out), on_progress=lambda p, pct, d: print(f"[{p}] {pct*100:.1f}%: {d}"), **vars(args))
